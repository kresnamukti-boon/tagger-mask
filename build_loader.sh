...
cat >> "$OUT" <<'FOOTER'
  ...
  console.log('[RW] workbench ready: ' + __RW.regions.filter(r=>r.included).length + ' regions. P=Pick K=Cut B=Rect N=Poly2 J=Brush A=Add O=Walls C=Pipe L=Elbow; ` undo, Escape cancel.');
})()
FOOTER
